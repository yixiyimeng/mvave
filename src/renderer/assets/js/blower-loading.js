...
// var LoadingBlower = (function() {
	...
	LoadingBlower.prototype.addLeaf = function() {
		var _this = this;
		if (this.leafCount > 0) {
			var timeOut = 150 + Math.random() * 150;
			setTimeout(function() {
...
function Plugin(action,arg) {
	return this.each(function() {
		var $this = $(this);
		// var options = $.extend({},  typeof option == 'object' && option);
		var data = $this.data('load');
		// var action = typeof option == 'string' ? option : NaN;
		if (!data) $this.data('load', (data = new LoadingBlower(this)));
		 if (action) data[action](arg);
	})
};


$.fn.LoadingBlower = Plugin;
$.fn.LoadingBlower.Constructor = LoadingBlower;
